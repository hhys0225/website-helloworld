      ...
      <el-main>
        ...
        <el-row v-for="(result,index) in info" :key="index">
          <el-col :span="22" :offset="2" style="padding-left: 30px; margin-bottom: 10px">
            <h3 class="t">
              <a target="_blank" style="float: left" :href="result.url" v-html="result.title"></a>
            </h3>
            <div style="width: 100%; padding-top: 5px; clear: both;">
              <div style="width: 600px; float: left">
                ...
                <div class="hc_contentfooter">
                  <span style="color: #666666; text-decoration: underline">获取结果</span>
                </div>
              </div>
            </div>
          </el-col>
        ...
        this.currentPage = 0;
          this.$axios
            ...
            .then(response => {
              ...
              this.info = response.data.data.result.splice(this.currentPage*10,10);
              this.loading = false;
            })
            .catch(err => {
              // alert("error");
